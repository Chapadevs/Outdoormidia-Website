...
app/
  ...
  area-do-anunciante/     — hub + sua-marca-no-ooh/, programatica/,
                            melhores-praticas/, diagnostico-de-presenca/ e faq/
...
| Nova campanha, dois WhatsApp — decisão de 09/09/2026: a porta de **Mídia Programática** voltou ao bloco, preta, apontando para o chat da MercadoOOH (`waLinkMercadoOoh(WA_PROGRAMATICA)`), e com ela a exceção de paleta voltou para o card que o checklist da home autoriza. **Atendimento rápido** perdeu o preto e o raio sobreposto e ficou na faixa clara translúcida do Diagnóstico, seguindo no comercial (`WA_ATENDIMENTO_AGORA`). Quem distingue os dois destinos é o ícone, não o rótulo: o balão **verde** (`icone-whatsapp-verde.png`) é o comercial da Outdoormídia, o balão com o **raio** (`whatsapp-mercadooh.png`) é a MercadoOOH. Os dois são asset de marca, então o quadrado do ícone é branco sólido nas duas portas, e não o `bg-white/15` do lucide: o verde apagaria contra o laranja da seção e o grafite do raio sumiria dentro do preto. É a exceção que o próprio checklist da home abre à monocromia laranja dos ícones ("Única exceção: o verde do WhatsApp") | `components/sections/NovaCampanha.jsx` |
...
| WhatsApp flutuante — dois botões empilhados no canto desde 09/09/2026, um por destino: o balão com o raio (`whatsapp-mercadooh.png`) leva ao chat da MercadoOOH, o balão verde (`icone-whatsapp-verde.png`) ao comercial da Outdoormídia, que fica embaixo por ser o primário. O verde é o único dos dois sem margem interna no arquivo, então renderiza a 36px contra os 48px do outro: nas duas medidas o desenho do balão sai igual. Botão flutuante não carrega rótulo, e é o ícone que diz qual é qual — foi por isso que o raio deixou de ser selo sobreposto ao balão verde e virou ícone inteiro | `components/widgets/WhatsAppButton.jsx` |
...
| Linha do tempo abaixo de 980px — a onda sai e entra a lista empilhada, que é o mesmo grid de cards de antes (duas colunas no tablet, uma no telefone). Posição absoluta em px não reflui, e continuar encolhendo o palco deixaria o texto do card ilegível muito antes de o layout quebrar. Só um dos dois está no documento por vez (`display:none`), então nada é lido duas vezes por leitor de tela. Sem medição (SSR, sem JS, `prefers-reduced-motion` ou abaixo de 980px) os quatro nascem acesos e a linha, inteira, em vez de a seção subir vazia | `components/sections/LinhaDoTempo.jsx` |
| Capa do topo de `/sobre` (10/09/2026) — a mesma foto da sede que abre o Banco de Talentos, o time reunido na frente do prédio, agora também na primeira dobra de Sobre nós. O arquivo é 2000×1125, 16/9 exato, então a capa sobe nessa proporção em vez do 16/7 que o slot previa: o corte de 16/7 cairia justamente nos pés de quem está agachado na ponta direita. Entra com `priority`, por ser a maior imagem da primeira dobra, e o alt vive nos quatro idiomas (`SobrePage.capaAlt`) | `app/[locale]/(site)/sobre/page.js`, `messages/*.json` |
...
| Programática na Área do anunciante — faixa larga escura abaixo da grade de cards, não um card a mais: ela não é ferramenta de autoatendimento, é frente comercial, e a hierarquia é o que diz isso. O kicker é derivado de `FERRAMENTAS.length`, e voltou a `4` com a entrada de Melhores Práticas; a faixa não entra na conta | `app/area-do-anunciante/page.js` |
...
| Mídia Programática — a porta do bloco Nova campanha aponta para a MercadoOOH, não para a página interna | O card voltou em 09/09/2026 com o destino que o briefing da reta final pede: o chat da MercadoOOH. O `checklist-programatica.md` (decorrência 02) pede outra coisa, trocar a linha de intenção para `Compro por DSP ou trading desk →` e apontar para `/area-do-anunciante/programatica`. Os dois documentos seguem se contradizendo, e o que está no ar é o briefing, que é o mais recente. Confirmar com o João se a porta deve levar ao chat da MercadoOOH ou à página interna. A página não fica órfã em nenhum dos casos: tem entrada pelo menu, pelo rodapé e pela faixa escura da Área do anunciante |
...
- Preto não faz parte da paleta. As exceções autorizadas são todas de Mídia
  Programática: a porta do bloco Nova campanha, o hero de
