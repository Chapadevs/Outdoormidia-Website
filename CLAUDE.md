...
| Nova campanha sem a porta de Mídia Programática (10/09/2026) — a porta preta que apontava para o chat da MercadoOOH (`waLinkMercadoOoh(WA_PROGRAMATICA)`) saiu do bloco a pedido do cliente, e com ela saiu a única exceção de paleta que o checklist da home autorizava aqui: o preto não aparece mais na seção. O bloco fica com Diagnóstico (faixa fina), o formulário (card dominante) e **Atendimento rápido**, que segue no comercial (`WA_ATENDIMENTO_AGORA`) na mesma faixa clara translúcida do Diagnóstico. O balão **verde** (`icone-whatsapp-verde.png`) é asset de marca, então o quadrado do ícone continua branco sólido, e não o `bg-white/15` do lucide: o verde apagaria contra o laranja da seção. É a exceção que o próprio checklist da home abre à monocromia laranja dos ícones ("Única exceção: o verde do WhatsApp"). As chaves `programaticaNome` e `programaticaIntencao` saíram dos quatro `messages/*.json`. A página `/area-do-anunciante/programatica` não fica órfã: tem entrada pelo menu, pelo rodapé e pela faixa escura da Área do anunciante, e o balão com o raio segue no botão flutuante do canto | `components/sections/NovaCampanha.jsx` |
...
| Mídia Programática — a porta saiu do bloco Nova campanha | Decisão do cliente em 10/09/2026 (ver Concluído). Encerra a contradição que estava aberta entre o briefing da reta final (porta preta para o chat da MercadoOOH) e o `checklist-programatica.md` (decorrência 02: linha de intenção `Compro por DSP ou trading desk →` apontando para `/area-do-anunciante/programatica`): nenhuma das duas está no ar. Se o João quiser a porta de volta, confirmar antes qual dos dois destinos vale |
