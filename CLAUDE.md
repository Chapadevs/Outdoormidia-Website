...
- **next-intl** — já implementado (ver "Idiomas" em Concluído); o que falta é conteúdo do Firestore (posts, cases, tags), que existe só em português
...
components/
  ...
  widgets/                — WhatsAppButton, RevealObserver, RadialReveal, DeleteButton,
                            LogoutButton, JsonLd, FaqJsonLd, Schema
lib/
  ...
  seo.js                  — PAGINAS (sitemap + llms.txt) e metaDe(), a metadata de
                            toda rota pública
  schema.js               — BreadcrumbList, Article e Service; o LocalBusiness fica
                            em widgets/JsonLd
...
| Idiomas (PT / EN / ES / ZH) — cobertura completa da copy do site (14/09/2026). O `next-intl` já roteava as quatro versões (`app/[locale]`, `messages/*.json`, overlays em `lib/i18n/content/`), mas várias páginas e componentes ainda traziam copy PT fixa no JSX. Nesta passagem tudo o que o visitante lê passou para `messages/*.json`: as páginas da Área do anunciante (hub, FAQ, Melhores práticas, Diagnóstico), do Blog (hub, Artigos, post, Podcast), Cases, Obrigado, Privacidade, Termos, Regiões, Trabalhe conosco, o hub e o template de Diferenciais, Tipos de mídia de `/solucoes`, os templates de plataforma e de Icônico, o 404, e os componentes `Ticker`, `PlatformsCatalog`, `FormatosGallery`, explorers do Blog e de Cases, `ShareButtons`, `LegalDoc`, `CoverMedia`, `CookieNotice`, `WhatsAppButton`, `ObrigadoCta`, `BlogTeaser`, `Cases`, `Faq`/`FaqCategorias`/`PlatformFaq`, `Culture`, `Process`, `HomeTimeline`, `ProdutoCard`, `MapaRodovias`, `VideoComSom`, `Iconicos`, `PraticasGrid`, `LinhaDoTempo`, `PostCard`, `CaseCard`, `ProposalForm`, `DiagnosticoQuiz` e `Reviews` (citação, cargo e contexto por posição em `Reviews.itens`; a fala é tradução da transcrição literal). Padrões: lista que só tem texto vira `t.raw('...')` por posição sobre a estrutura que não se traduz (ícone, href); rótulo que vira valor de lead segue em PT (`ProposalForm` guarda `valor` PT e mostra `rotulo` traduzido, como o `QualifierForm` já fazia); data longa sai de `dataLonga(locale)` em `lib/format.js`, porque `DATA_LONGA` é fixo em pt-BR; tempo de leitura vem de `readingTimeMinutes` mais a mensagem `Blog.minLeitura`. `Breadcrumb` ficou com o "Home" fixo de propósito: o painel `/admin` o renderiza dentro de um provider sem mensagens. `npm run check:i18n` confere que os quatro JSON têm o mesmo conjunto de chaves | `messages/*.json`, `lib/format.js`, `lib/blog/readingTime.js` |
...
| Setas dos Icônicos nas laterais e animação reduzida (14/09/2026) — as duas setas saíram do cabeçalho da faixa (título + régua ficam sozinhos) e da linha do contador, e passaram a viver **fora do card branco**, nas laterais do palco, centradas na altura do card: 56px, borda e chevron brancos sobre o laranja, com Radial Reveal em branco (o palco reserva `px-[76px]` para elas, e o `z-[3]` as põe acima das camadas do `IconicosFx`). Abaixo de 980px o palco perde a faixa lateral e as setas voltam para a linha do contador, ao lado do `01 / 07`, no mesmo desenho branco. Na mesma passagem, a pedido do cliente, o volume das animações caiu: Green de 34 folhas (22 a 56px), 6 caindo e 5 cipós para 14 folhas (16 a 34px), 3 caindo e só os 2 cipós de moldura, com halo e tinge mais fracos; Regenerativo de 3 ondas e 4 partículas para 1 e 2, grade mais discreta; Elegancy de 10 estrelas para 6, menores. O que anima e como não mudou, só a quantidade e a força | `components/sections/Iconicos.jsx`, `components/ui/IconicosFx.jsx` |
| Icônicos com carrossel de ativos (13/09/2026) — a coluna da direita da faixa laranja deixou de ser a foto única da linha (com as animações do `IconicosFx`) e virou o carrossel dos ativos da linha aberta: um `AtivoCard` por vez, o mesmo card do grid que existia abaixo da faixa, com contador `01 / 07` e setas que passam de um ativo ao outro dentro da linha, dando a volta nas pontas. As abas Green · Regenerativo · Elegancy trocam de linha e reabrem no primeiro ativo; as setas do cabeçalho passaram a andar pelos cards, não pelas linhas. Todos os 16 cards das três linhas saem no HTML e só `hidden` escolhe qual aparece, então o texto segue no documento e a âncora de ativo (`#jardim-digital`) abre linha e card certos; a âncora de linha (`#green`) mora na própria aba. **A seção clara de ativos abaixo da faixa saiu** (e com ela o prop `comAtivos`): mostrava a mesma lista. `frase` e `ctaLinha` de cada linha em `lib/iconicos.js` ficaram sem call site. **As animações voltaram em 14/09/2026, em volta do card** (ver a entrada "Animações da foto dos Icônicos"); só o `SlideStage` segue no repo sem uso, como o `Coverflow` | `components/sections/Iconicos.jsx`, `app/[locale]/(site)/plataformas/projetos-iconicos/page.js` |
...
| Revisão de SEO/GEO (14/09/2026) — toda rota pública monta a metadata por `metaDe()` em `lib/seo.js` (title, description, canonical + hreflang, Open Graph com `url`, `siteName` e `locale`, e o slot de `imagem` para quando o asset de OG chegar), no lugar do bloco que estava copiado em 27 páginas e perdia `og:url` e `og:site_name` (o `openGraph` da página substitui o do layout inteiro, o Next não faz merge profundo). A home ganhou hreflang, que só o layout declarava como canonical, e o canonical saiu do layout para a 404 não herdar um apontando para a home. `robots` global libera `max-image-preview: large` e `max-snippet: -1`. **Post do blog só existe em português:** `somentePt` faz o canonical de `/en`, `/es` e `/zh` apontar para a URL sem prefixo, sem hreflang, e o sitemap lista o post uma vez, porque anunciar `/en/blog/x` como versão inglesa de um texto em PT é hreflang inválido. O `lastmod` das rotas estáticas do sitemap é a data do build (`env.BUILD_DATE` em `next.config.mjs`), não a hora da requisição, que dizia ao Google que as 140 URLs mudavam a cada hora. Structured data novo em `lib/schema.js`, injetado por `components/widgets/Schema.jsx` (escapa `<` para o título do CMS não fechar o `<script>`): `BreadcrumbList` dentro do próprio `Breadcrumb`, `Article` no post, `Service` nas 8 plataformas, todos apontando para o `#negocio` do `LocalBusiness`. `EMPRESA.redes` virou objeto e é a fonte única do rodapé e do `sameAs` (o X, que já tinha saído do rodapé, seguia no JSON-LD; o YouTube entrou); `areaServida` nomeia Joinville, Itajaí e Balneário Camboriú; `geo` da sede é slot vazio sob `TODO(cliente)`. O `llms.txt` passou a listar os artigos publicados (mesma leitura tolerante a falha do sitemap) e as redes sociais | `lib/seo.js`, `lib/schema.js`, `components/widgets/Schema.jsx`, `components/ui/Breadcrumb.jsx`, `app/[locale]/layout.js`, `app/sitemap.js`, `app/llms.txt/route.js`, `lib/empresa.js`, `next.config.mjs` |
...
| Google Analytics 4 com opt-in (14/09/2026) — propriedade do cliente `G-H0X3RJE0Z1`, ID lido de `NEXT_PUBLIC_GA_MEASUREMENT_ID` (definido só em `apphosting.yaml`; vazio em dev, e sem ele nada carrega). O `gtag` entra por `components/widgets/Analytics.jsx`, sem dependência nova, e **só depois do Aceitar** no aviso de cookies: a escolha vive em `lib/consentimento.js` (`om-consentimento` = `aceito`/`recusado`; a chave antiga `om-aviso-cookies` vale como aviso lido, nunca como consentimento, porque aquele aviso prometia "sem rastreamento"). Quem recusa navega sem nenhum script de medição. O `page_view` é manual (`send_page_view: false`), disparado a cada troca de `pathname`, porque o App Router não recarrega. Dois eventos de conversão: `gerar_lead` em `/obrigado` (o `ObrigadoCta` dispara na montagem, com `origem`) e `contato_whatsapp`, capturado por um ouvinte de clique no documento sobre qualquer `<a href="https://wa.me/…">`, com a rota e o rótulo do botão, sem tocar em call site. O `CookieNotice` ganhou os dois botões (`cookiesAceitar`/`cookiesRecusar` nos quatro `messages/*.json`, `cookiesEntendi` saiu) e a Política de Privacidade (`lib/legal.js` e as três traduções) passou a declarar os cookies de medição e como retirar o aceite | `components/widgets/Analytics.jsx`, `components/widgets/CookieNotice.jsx`, `lib/analytics.js`, `lib/consentimento.js`, `lib/legal.js`, `apphosting.yaml` |
...
| `outdoor-digital.mp4` trocado, reencodado e versionado (14/09/2026) — o vídeo novo do cliente entrou em 13/09 **no mesmo caminho** do anterior, em 4K a 15 Mbps (15,6 MB), e a página seguia mostrando o antigo: `/media/` sai com cache de sete dias, mesma lição do `?v=2` de Digital Signage e do WebP de Face Única. Passou pela receita da casa (H.264, 1080p, CRF 26, teto de 2,5 Mbps, sem áudio, `faststart`): 2,75 MB, mesmos 490 quadros a 59,94 fps. A URL subiu para `?v=2` em `lib/platforms.js`, o pôster `outdoor-digital-capa.webp` foi regerado a partir do quadro 0 do vídeo novo (o `posterDoVideo` carrega a query junto), e o original de 4K foi para `public/media/images/plataformas/outdoor-digital-4k-original.mp4`, fora do versionamento | `lib/platforms.js`, `public/media/plataformas/outdoor-digital.mp4`, `public/media/plataformas/outdoor-digital-capa.webp` |
...
| Botão "Falar com o time comercial" no hero das 8 páginas de plataforma (13/09/2026) — `btn-fill` abaixo do parágrafo de abertura, na coluna de texto ao lado da capa. Abre o WhatsApp com `waPlataforma`: `Olá! Tenho interesse em anunciar na plataforma X.`, sempre com o nome PT do catálogo (`getPlatformBySlug`), que é o que o comercial reconhece, e a etiqueta de `comIdioma` nos outros idiomas. O rótulo do botão vive em `PlatformPage.falarComercial` nos quatro `messages/*.json` | `app/[locale]/(site)/plataformas/[slug]/page.js`, `lib/whatsapp.js` |
| Empena Digital em `/plataformas/outdoor-digital` (14/09/2026) — seção própria abaixo dos Produtos, com o texto do cliente verbatim e a foto da empena (dois painéis de LED na lateral de um edifício) em 16/9 ao lado do argumento. Entra pelo campo `destaque` da entrada da plataforma (`title`, `text`, `image`, `imageAlt`), genérico para qualquer formato sem ficha de catálogo: sem ele a seção não sobe. Não é produto de `lib/produtos.js` porque a empena não tem specs. O PNG de 4000×2250 foi para `public/media/images/produtos/`, fora do versionamento; o site serve `empena-digital.webp` (2000px, 76 KB). **Ressalva:** a foto traz a marca d'água `nova ooh` no canto e a campanha da Johnnie Walker; confirmar com o cliente o direito de uso antes de publicar | `lib/platforms.js`, `lib/i18n/content/platforms.*.js`, `app/[locale]/(site)/plataformas/[slug]/page.js`, `public/media/produtos/empena-digital.webp` |
...
| Dois cards abaixo do mapa de Rodovias (13/09/2026) — os dois links do Google My Maps que o handoff previa, cada um como card com a captura do próprio mapa como capa: **Painéis instalados** (os painéis já em operação) e **Simulações sob demanda** (os pontos onde o painel pode ser construído), com o botão de cada um abrindo o viewer em nova aba. URL só em modo viewer, sem `/edit`, e nunca iframe, como manda o handoff. As capturas têm proporções opostas (530×695 e 670×529) e os marcadores vão até a borda, então a capa é uma caixa quadrada com a imagem contida sobre `bg-bone`: cortar em `object-cover` levaria Curitiba ou Itajaí. Os links e as capas vivem em `mapasExternos` na entrada da plataforma; título, texto, rótulo do botão e alt vivem em `PlatformPage.mapasExternos` nos quatro `messages/*.json`. Os PNG originais (com espaço e acento no nome) foram para `public/media/images/rodovias/`, fora do versionamento; o site serve os WebP em `public/media/rodovias/`, 107 KB ao todo | `app/[locale]/(site)/plataformas/[slug]/page.js`, `lib/platforms.js`, `messages/*.json`, `public/media/rodovias/` |
...
| Mídia Indoor em `/plataformas/midia-indoor` (14/09/2026) — o slug `shoppings`, herdado da primeira versão do catálogo, saiu a pedido do cliente. A rota antiga é redirect permanente (`next.config.mjs`, com o par de locale). Case gravado no Firestore com `platforms: ['shoppings']` continua valendo: `SLUGS_LEGADOS` em `lib/platforms.js` é a fonte única, `slugsDaPlataforma` faz a página nova encontrar o case antigo, o `validate` de cases aceita o slug velho e o editor do admin o normaliza com `slugAtual`, então o case é regravado com o slug novo ao salvar. A pasta `public/media/shoppings/` não mudou de nome: nome de asset não é copy. Na mesma passagem, a foto do Totem no card de produto deixou de subir cortada: o acervo é retrato (900×1600) e a caixa 16/9 mostrava só o meio da tela; entrou `produto-totem-4x3.webp` (corte que enquadra a tela inteira e o topo da estrutura) com `imageRatio: '4/3'`, em arquivo de nome novo pela regra de cache de `/media/` | `lib/platforms.js`, `lib/cases/cases.js`, `lib/cases/validate.js`, `components/forms/CaseEditorForm.jsx`, `next.config.mjs`, `lib/produtos.js` |
| Foto nos 3 produtos de MUB no card de produto (13/09/2026) — Banca Horizontal, Banca Vertical e Relógio Digital subiam sem `image` em `lib/produtos.js` e o card ficava sem capa. As três fotos já existiam no repo (`public/media/tipos-de-midia/`, usadas na tabela de tipos de mídia de `/solucoes`) e foram reaproveitadas como `image`/`imageAlt`, sem gerar arquivo novo | `lib/produtos.js` |
| Foto nos 2 produtos de Mídia Móvel no card de produto (13/09/2026) — Bike Mídia (o trio no gramado do Parque Barigui, com a roda-gigante ao fundo) e Bus Mídia (traseira de ônibus com a lona ocupando o vidro inteiro) ganharam `image`/`imageAlt` em `lib/produtos.js`, com alt nos quatro idiomas. Os dois cards sobem em `imageRatio: '4/3'`: a foto do ônibus é retrato (864×1152) e foi recortada de cima para 864×648, ficando com o anúncio e o número do ônibus, porque em 16/9 sobraria só a lona sem o ônibus em volta; a da Bike acompanha a proporção para os dois cards ficarem da mesma altura, e como ela é 16/9 inteira (1600×900) o corte central do `object-cover` mantém o trio, que está no centro do quadro. Os originais (`BIKE MIDIA.JPG` de 8064×4536 e 29 MB, `BUSDOOR.jpeg`) foram para `public/media/images/midia-movel/`, fora do versionamento; o site serve os WebP em `public/media/midia-movel/` (432 KB ao todo). São as mesmas duas fotos que `/solucoes` já usa na tabela de tipos de mídia, só que lá ainda são servidas pelos arquivos originais de `public/media/tipos-de-midia/`, o JPG de 29 MB incluído | `lib/produtos.js`, `lib/i18n/content/produtos.*.js`, `public/media/midia-movel/` |
...
| Fotos reais nos três Formatos de Digital Signage (14/09/2026, à tarde) — as três peças recortadas de 13/09 (`fachada-digital.webp`, `posto-de-combustivel.webp`, `passagem-digital.webp`) saíram e entraram três fotos reais do cliente, uma por card: o totem em frente à loja da Vanguard na Fachada digital, o painel na fachada da Audiotec no Posto de combustível e o painel junto ao acesso da clínica Samanta Nigro na Passagem digital. São fotografias inteiras, sem fundo transparente, então o `FormatSpecCard` perdeu o `recorte` e a foto sobe com a moldura normal do `CoverMedia`; os WebP já saem cortados em 3/4 (a proporção do card, 900×1200, 466 KB ao todo), com o corte escolhido para manter o painel inteiro. Nome novo com sufixo `-v2`, pela regra do cache de sete dias de `/media/`. Os PNG (`digital signage 1.png`, `2`, `3`) foram para `public/media/images/digital-signage/`, fora do versionamento. Alt novo nos quatro idiomas. **Ressalva mantida:** nenhuma das três é de posto de combustível; a do card do Posto é a da fachada da Audiotec. Trocar quando o cliente mandar foto de posto | `components/ui/FormatSpecCard.jsx`, `lib/platforms.js`, `lib/i18n/content/platforms.*.js`, `public/media/digital-signage/` |
| Fotos nos três produtos de Digital Signage (13/09/2026, à noite) — as três peças recortadas do cliente (`1.png`, `2.png` e `3.png` em `public/media/images/digital-signage/`, fora do versionamento) voltaram à página, agora uma por card de Formatos: `fachada-digital.webp` na Fachada digital, `posto-de-combustivel.webp` no Posto de combustível e `passagem-digital.webp` na Passagem digital, servidas em 1600px de altura com alfa, 500 KB ao todo. `FormatSpecCard` ganhou o campo `image`: com ele a foto toma o lugar do diagrama de cotas, que em Digital Signage não tinha medida real para mostrar (`Sob medida` dos dois lados). Sobem com `recorte`, porque o arco laranja e o fundo transparente já são a moldura. O overlay de idioma mescla por índice, então só `imageAlt` entrou nos três `platforms.*.js`. **Correção de 14/09/2026:** as fotos nunca chegaram a aparecer, porque Produtos e Formatos eram uma seção exclusiva no template e Digital Signage tem produto de catálogo (Painel Exclusivo), que ganhava do `FormatSpecCard`. Agora formato com `image` sobe como bloco Formatos abaixo dos Produtos (`formatosIlustrados`); sem foto, o produto segue substituindo o diagrama. Na mesma passagem entraram os `quandoIcones` (Building2, Megaphone, Network), que era a única das sete plataformas sem ícone nos cards de "Quando essa plataforma é a escolha certa". **Ressalva:** nenhuma das três peças é de posto de combustível; as duas verticais são o mesmo totem em frente a prédio, e a que ficou no card do Posto é a de fundo claro. Trocar quando o cliente mandar foto de posto. O hero e o card do carrossel seguem só com o vídeo (linha seguinte) | `components/ui/FormatSpecCard.jsx`, `lib/platforms.js`, `lib/i18n/content/platforms.*.js`, `public/media/digital-signage/` |
| Digital Signage só com vídeo (13/09/2026) — as três fotos recortadas do cliente (a fachada digital no hero e as duas da faixa de duas colunas abaixo de "Quando essa plataforma é a escolha certa") saíram da página a pedido dele; a única mídia da plataforma passou a ser o `digital-signage.mp4`, o mesmo arquivo no hero (16/10 com moldura, como Front Light e Outdoor Digital, no lugar da caixa 3/4 sem moldura em que o vídeo estava sendo cortado em retrato) e no card do carrossel da home e de `/solucoes` (via `video` em `PLATFORMS_LISTAGEM`, com o pôster `digital-signage-capa.webp` já gerado). O vídeo já estava na receita da casa desde 09/09/2026 (1080p, 2,24 Mbps, sem áudio, `faststart`), e o original de 11,7 Mbps segue em `public/media/images/plataformas/`, fora do versionamento. Saíram junto os três WebP servidos, `imageAlt`/`galeria` dos três overlays de idioma (overlay de lista sem base **ressuscitaria** a galeria naquele idioma, porque `aplicarOverlay` devolve o overlay quando a base não é array) e o bloco de `galeria` do template, que ficou sem entrada que o alimentasse. `imageRecorte` e o `recorte` do `CoverMedia` continuam existindo (o logo da Eco Local Recicla usa o segundo). O cartaz "Empresas que atendemos" não é foto de painel e ficou (ver a linha seguinte). Os PNG originais seguem em `public/media/images/digital-signage/` | `lib/platforms.js`, `lib/i18n/content/platforms.*.js`, `app/[locale]/(site)/plataformas/[slug]/page.js`, `components/ui/CoverMedia.jsx` |
...
| Linha do tempo — cinco marcos (1959 · 1980s · 2000s · 2010 · Hoje). Em 12/09/2026 o cliente reescreveu 2000s (a troca de geração da empresa) e devolveu o marco de 2010 (pioneiros no outdoor digital, o maior roteiro de projetos icônicos do Sul), que tinha sido fundido ao anterior em 25/08. Os dois textos entraram verbatim, inclusive a concordância de "Os anos 2000 marcou"; os títulos dos dois cards são nossos, porque a copy veio sem título. 2010 subiu sem foto até 13/09/2026, quando entrou a banca digital do cliente (`BANCA HORIZONTAL.jpg`, 5472×3648, servida como `2010.webp` 1600×900, corte centralizado; o original foi para `public/media/images/linha-do-tempo/`, fora do versionamento), com alt nos quatro idiomas. Os cinco marcos têm foto | `lib/sobre.js`, `lib/i18n/content/sobre.*.js`, `public/media/linha-do-tempo/2010.webp` |
...
| Banco Reciclável em Ambiental (12/09/2026) — nova seção entre "Ciclo da lona" e "Doação Instituto Kopher": os bancos de concreto reciclado que a Outdoormídia instala em praças e calçadas, em parceria com a Eco Local Recicla, com a foto real (banco já instalado, com a placa da marca) e o logo do parceiro. Não entrou como quarto card da seção 01 (`AMBIENTAL_REALIDADE`): aquela seção é a carteira Mídia Regenerativa, três ativos com endereço único, e o banco é prática recorrente em várias praças, sem endereço próprio — por isso virou seção dedicada, no mesmo padrão de "Ciclo da lona" e "Doação Instituto Kopher" (card com mídia de um lado, texto do outro). A foto do acervo é 4:3, não 16:9, então `CoverMedia` ganhou a proporção `4/3` no mapa estático de `RATIOS`. O logo do parceiro sobe com `recorte` (contido, sem corte) porque a arte não tem margem em volta do texto, e cortar em `object-cover` cortaria a palavra "RECICLA" | `app/[locale]/(site)/sobre/ambiental/page.js`, `components/ui/CoverMedia.jsx`, `messages/*.json`, `public/media/ambiental/banco-reciclavel.webp`, `public/media/ambiental/logo-reciclavel.png` |
...
| Comparação Amador / Especialista de Face Única em WebP com nome novo (14/09/2026) — as duas fotos da seção `#comparacao` tinham sido trocadas em 14/09 **no mesmo caminho** (`face-unica-1.png` e `face-unica-2.png`, 7,8 MB e 2,6 MB de PNG servidos sem conversão), e a produção seguiu mostrando as antigas: `/media/` sai com `Cache-Control: public, max-age=604800`, e o CDN do App Hosting guarda o arquivo por sete dias, então arquivo novo no caminho velho não chega a quem já abriu a página. Mesma lição do `?v=2` do vídeo de Digital Signage, resolvida aqui pelo caminho: `face-unica-amador.webp` (2000×1000, 232 KB) e `face-unica-especialista.webp` (1584×993, 286 KB), sem alfa (o canal alfa do mockup era todo opaco). Os PNG foram para `public/media/images/diferenciais/`, fora do versionamento. **Regra:** ao trocar um arquivo de `/media/`, trocar o nome ou subir o `?v=`; nunca substituir no mesmo caminho | `lib/diferenciais.js`, `public/media/diferenciais/face-unica-amador.webp`, `public/media/diferenciais/face-unica-especialista.webp` |
| Capa de Face Única é a estrutura tripla do Colégio Marista (13/09/2026) — a foto real de três outdoors à beira de avenida em Curitiba inteiramente ocupados pela campanha de matrículas do Marista: é o conceito da página em uma imagem só. Substituiu o `distrito-de-midia.png` de 08/09/2026 (17 MB, servido sem conversão), que saiu do versionamento e foi para `public/media/images/diferenciais/`, junto com o JPEG original de 6240×4160 desta foto. O site serve o WebP de 2000×1333, 431 KB; a foto é 3:2 e o `CoverMedia` corta por `object-cover` nas caixas 16/9 do card e do hero, com o painel no centro. O alt vive nos quatro idiomas. Antes disso, a capa de 06/09 era o recorte da direita de outra foto, feito para não republicar o "+380 milhões de impactos mensais" impresso nos painéis da esquerda | `lib/diferenciais.js`, `lib/i18n/content/diferenciais.*.js`, `public/media/diferenciais/face-unica.webp` |
...
| Google Analytics — conversões e filtro interno | O GA4 está instalado (ver Concluído). Falta, dentro da propriedade `G-H0X3RJE0Z1`: marcar `gerar_lead` e `contato_whatsapp` como conversão (Administrador → Eventos), criar o filtro de tráfego interno para o hostname `*.hosted.app` e vincular o Search Console. Confirmar com o cliente se o site antigo usa o mesmo `G-`: se usar, os dois sites caem na mesma propriedade enquanto coexistirem |
| Imagem de Open Graph | Não há asset de OG em `public/`, então link compartilhado sai sem card. Desde 14/09/2026 `metaDe()` tem o parâmetro `imagem`: quando o asset vier, uma imagem padrão passada ali (ou em `app/[locale]/layout.js`) entra em todas as rotas de uma vez, e o `twitter:card` vira `summary_large_image`. `logo.png` e `om.png` são brancos (viram máscara CSS), não servem como `logo` de schema |
| Titles e descriptions de busca | Os 23 titles de `Meta` nos `messages/*.json` são o nome da página mais a marca (`FAQ | Outdoormídia`, `Plataformas | Outdoormídia`), sem termo de busca; só Regiões e Melhores Práticas foram trabalhados. As 8 plataformas saem como `Nome | Outdoormídia`, e o caminho é o mesmo dos diferenciais: campo `seo.title` por entrada em `lib/platforms.js`. Dez descriptions passam de 160 caracteres (Ambiental 209, Diferenciais 188, Governança 184, Social 176, Termos 172, Privacidade 171, Trabalhe Conosco 167, Sobre 164, Soluções 163, FAQ 162). É copy, depende de aprovação do cliente |
| Coordenadas da sede no JSON-LD | `EMPRESA.geo` está vazio sob `TODO(cliente)`; com latitude e longitude o `LocalBusiness` ganha `geo`, que é o que amarra o negócio ao ponto no mapa na busca local |
| Idiomas (PT / EN / ES / ZH) — o que ainda é só PT | Conteúdo do Firestore (posts do blog, cases e nomes de tag) existe só em português: o canonical de `/en`, `/es` e `/zh` nos posts aponta para a URL sem prefixo (`somentePt` em `lib/seo.js`). Ficam em PT de propósito, porque vão para o comercial ou para o schema: o valor das opções do qualificador e do `ProposalForm`, o `contexto` do lead, as mensagens de WhatsApp de `lib/whatsapp.js`, o JSON-LD de `JsonLd.jsx` e o painel `/admin`. Os vídeos de depoimento seguem com a legenda embutida em português |
...
| Fotos dos diferenciais 01 e 02 | As capas do topo das duas chegaram em 06/09/2026 e os dois lados da comparação de Face Única (Amador e Especialista) estão no ar desde 08/09, trocados em 14/09 (ver Concluído): Face Única não está mais bloqueada por foto. Aeroporto Square está publicável, e o que falta é a foto da seção "O que é", ilustrando o conceito híbrido. Regra da base para todas: foto real, painel 100% limpo e visível, sem filtro ou IA que altere cor ou proporção, sem fio elétrico na frente, produto nunca cortado |
...
3. **Mídia Indoor** (slug `midia-indoor`) — São José, Park Shopping Boulevard e Itália (100% digital, totens e painéis)
...
**Mídia Indoor** atendeu por `/plataformas/shoppings` até 14/09/2026; hoje é
`/plataformas/midia-indoor`, com redirect da rota antiga e tolerância ao slug
velho nos cases (ver Concluído). **Gentileza Urbana** saiu do catálogo de plataformas e, em
...
Ambiental chama-se **Mídia Regenerativa**, e a tag de cada card é o nome da linha de
Icônicos a que o ativo pertence: **Regenerativo** nas duas praças e **Green** no
Jardim Vertical (corrigido em 13/09/2026, o card subia como Regenerativo). `Gentileza Urbana` sobrevive só na
