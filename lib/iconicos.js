...
// `ICONICOS_ASSINATURA` são os quatro ativos de assinatura, hoje instalados na
// linha Elegancy; `ICONICOS` são as três linhas, e cada linha carrega os ativos
// já instalados.
//
// Cada linha alimenta três lugares: os campos de topo (`name`,
...
// Os quatro ativos de assinatura. Desde 06/09/2026 eles entram na linha
// Elegancy, que é onde o mobiliário de assinatura já morava; a lista continua
// separada porque três deles estão no Distrito de Mídia do Aeroporto e
// aparecem também em /plataformas/aeroporto, lendo esta mesma lista.
export const ICONICOS_ASSINATURA = [
  {
    ...
    image: '/media/aeroporto/aeroporto-square.jpeg',
    imageAlt:
...
export const ICONICOS = [
  ...
  },
  {
    ...
    ativos: [
      ...
      // Deixou de ser Praça Pet no briefing da reta final: o eixo da
      // comunicação passou a ser conveniência, e o que o card destaca agora é
      // o produto côncavo, o primeiro de Curitiba. Segue sendo regenerativa,
      // por isso continua nesta linha, mas não é mais isso que ela anuncia.
      {
        slug: 'praca-conveniencia-batel',
        name: 'Praça de Conveniência Batel',
        kicker: 'Primeiro produto côncavo de Curitiba',
        text: 'Espaço planejado para lazer, bem-estar e inclusão, com infraestrutura segura e moderna que revitaliza o entorno e cria um novo ponto de encontro entre marcas, pessoas e cidade. É também o primeiro produto côncavo de Curitiba: a face curva acompanha a esquina e entrega a marca em ângulo aberto, de quem chega pelas duas vias.',
      ...
      },
      {
        ...
        text: 'A terceira Praça Pet do projeto Regenerativo. Consolida o compromisso da marca patrocinadora com a valorização de espaços públicos, ampliando o legado iniciado nas duas praças anteriores.',
  ...
  },
  {
    ...
    short:
      'Design autoral em endereços nobres: a mídia soma à paisagem em vez de disputar com ela.',
    ...
    ativos: [
      ...
      // Os quatro ícones de assinatura passaram a viver aqui: eles não tinham
      // linha e ocupavam uma seção só deles na página de Icônicos, com a mesma
      // ideia que Elegancy já nomeia. O texto continua tendo um dono só, em
      // `ICONICOS_ASSINATURA`, porque `/plataformas/aeroporto` espelha três
      // deles por slug.
      ...ICONICOS_ASSINATURA,
    ],
...
// Versao de cada idioma. Nome de linha e de ativo nao entram no overlay: nome
// oficial nao se traduz. Slug, href, imagem e endereco tambem continuam aqui.
import { porLocale } from '@/lib/i18n/overlay'
import * as icEn from '@/lib/i18n/content/iconicos.en'
import * as icEs from '@/lib/i18n/content/iconicos.es'
import * as icZh from '@/lib/i18n/content/iconicos.zh'

const conteudoIconicos = porLocale(ICONICOS, {
  en: icEn.ICONICOS,
  es: icEs.ICONICOS,
  zh: icZh.ICONICOS,
})
const conteudoAssinatura = porLocale(ICONICOS_ASSINATURA, {
  en: icEn.ICONICOS_ASSINATURA,
  es: icEs.ICONICOS_ASSINATURA,
  zh: icZh.ICONICOS_ASSINATURA,
})

const VER_NA_LINHA = {
  pt: 'Ver na linha {linha}',
  en: icEn.VER_NA_LINHA,
  es: icEs.VER_NA_LINHA,
  zh: icZh.VER_NA_LINHA,
}

export function getIconicos(locale) {
  return conteudoIconicos(locale)
}

export function getIconicosAssinatura(locale) {
  return conteudoAssinatura(locale)
}

export function getIconicoBySlugLocale(slug, locale) {
  return conteudoIconicos(locale).find((i) => i.slug === slug)
}

export function getOutrosIconicosLocale(slug, locale) {
  return conteudoIconicos(locale).filter((i) => i.slug !== slug)
}

// Mesma busca de getAtivoBySlug, no idioma pedido. O rotulo do link de volta e
// montado aqui porque so aqui se sabe de que linha o ativo veio.
export function getAtivoBySlugLocale(slug, locale = 'pt') {
  const daAssinatura = conteudoAssinatura(locale).find((a) => a.slug === slug)
  if (daAssinatura) return daAssinatura

  const modelo = VER_NA_LINHA[locale] ?? VER_NA_LINHA.pt

  for (const linha of conteudoIconicos(locale)) {
    const ativo = linha.ativos.find((a) => a.slug === slug)
    if (ativo) {
      return {
        ...ativo,
        verEm: {
          label: modelo.replace('{linha}', linha.name),
          href: `/plataformas/projetos-iconicos#${linha.slug}`,
        },
      }
    }
