...
function toDoc({ origem, nome, empresa, cnpj, email, whatsapp, dados }) {
  const permitidos = ORIGENS[origem].campos.map((c) => c.key)
  return {
    origem,
    nome: texto(nome),
    empresa: texto(empresa),
    // Guardado já formatado (XX.XXX.XXX/XXXX-XX): é assim que o comercial lê
    // e copia. A validação garante que só entra CNPJ completo e válido.
