...
import { PLATFORMS, SLUGS_LEGADOS } from '@/lib/platforms'
import { ICONICOS } from '@/lib/iconicos'

// Os icônicos entram junto: saíram do catálogo, mas continuam sendo destino
// válido de um case (e há cases gravados antes da retaxonomia).
const PLATFORM_SLUGS = new Set(
  [...PLATFORMS, ...ICONICOS]
    .map((p) => p.slug)
    .concat('projetos-iconicos', ...Object.values(SLUGS_LEGADOS).flat())
)
