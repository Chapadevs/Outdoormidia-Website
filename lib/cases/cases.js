...
import { slugsDaPlataforma } from '@/lib/platforms'
...
// A ordenação é em JS de propósito: `where('status') + orderBy('publishedAt')`
// exigiria um índice composto no Firestore. Não trocar por orderBy sem criar o
// índice antes — a query passa a falhar em runtime, não no build.
...
export async function getPublishedCasesByPlatform(platformSlug) {
  ...
  const slugs = slugsDaPlataforma(platformSlug)
  return cases.filter((c) => c.platforms.some((p) => slugs.includes(p)))
}
...
// Ainda não existe /cases/[slug] — a unicidade é mantida mesmo assim, para que
// a página de detalhe possa ser criada depois sem encontrar slugs colididos.
