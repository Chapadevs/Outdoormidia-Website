...
export const PLATFORMS = [
  {
    ...
    // Página de painel digital não sobe painel estático: `lib/produtos.js`
    // restringe cada produto a esta tecnologia (foto, pontos e texto).
    tecnologia: 'digital',
    intro:
    ...
    // O `?v=` existe porque o arquivo foi trocado por outro vídeo (13/09/2026)
    // no mesmo caminho: `/media/` sai com cache de sete dias, e sem uma URL
    // nova quem já tinha aberto a página seguia vendo o vídeo anterior.
    // Subir a versão ao trocar o arquivo de novo.
    video: '/media/plataformas/outdoor-digital.mp4?v=2',
    ...
    // Seção própria da Empena Digital (14/09/2026), abaixo dos Produtos: texto
    // do cliente verbatim e a foto em 16/9. Fica fora de `lib/produtos.js`
    // porque a empena não tem ficha técnica de catálogo, só o argumento.
    destaque: {
      title: 'Empena Digital',
      text: 'Transforme grandes fachadas e laterais de edifícios em pontos de alto impacto para sua marca. As Empenas Outdoormídia oferecem ampla área de exposição, presença marcante e visibilidade privilegiada em locais estratégicos da cidade.',
      image: '/media/produtos/empena-digital.webp',
      imageAlt:
        'Empena digital da Outdoormídia na lateral de um edifício ao entardecer, com dois painéis de LED acesos exibindo a mesma campanha',
    },
    faqs: [
      {
  ...
  },
  {
    ...
    heading: 'Front Light.',
    // Página de painel estático não sobe painel digital: mesma restrição de
    // Outdoor Digital, no sentido inverso. Produto que só tem foto digital
    // (Poster Sight, Billboard) segue com ela, porque é a única que existe.
    tecnologia: 'estatico',
    intro:
    ...
    quandoIcones: [Repeat, MapIcon, Route],
  ...
  },
  {
    ...
    quandoIcones: [ShoppingCart, Sparkles, Repeat],
    bignumbers: [
  ...
  },
  {
    ...
    quandoIcones: [Briefcase, Landmark, Building2],
    bignumbers: [
      { n: '577,5 m²', label: 'Área visual' },
      { n: '800 mil', label: 'Impactos por mês' },
      { n: '14,8 mi', label: 'Passageiros por ano' },
      { n: '3,7 mi', label: 'Habitantes na região' },
    ],
    blocosTitle: 'O Distrito',
    // Fora de `blocos` pela mesma razão de `quandoIcones`: o overlay de idioma
    // troca o array inteiro, e o ícone não é texto.
    blocosIcones: [MapPin, Users],
    blocos: [
      {
        title: 'Onde está',
  ...
  },
  {
    ...
    quandoIcones: [CalendarDays, Footprints, Megaphone],
  ...
  // interno e bastam para o card e a página existirem.
  {
    ...
    heading: 'Rodovias.',
    intro:
      'Os principais corredores do Sul do Brasil em uma única rede. De Ponta Grossa a Florianópolis, passando pelo litoral e por Joinville, os painéis Rodovias acompanham o fluxo diário entre as duas maiores regiões econômicas do Sul: trabalho, turismo e logística no mesmo trajeto.',
    ...
    mapaRede: true,
    // Os dois mapas do Google My Maps que o handoff previa abaixo da rede, com a
    // captura de cada um como capa do card. Só URL em modo viewer, nunca `/edit`
    // e nunca como iframe (regra do handoff). Os textos vivem em
    // `PlatformPage.mapasExternos` nos quatro messages/*.json, chaveados pelo id.
    mapasExternos: [
      {
        id: 'paineis',
        image: '/media/rodovias/paineis-instalados.webp',
        href: 'https://www.google.com/maps/d/viewer?mid=1fIbDsbDnAIJ7hdJyjS9Gbc9q9adleEg&ll=-25.915778530821704%2C-49.422869115&z=8',
      },
      {
        id: 'simulacoes',
        image: '/media/rodovias/simulacoes-sob-demanda.webp',
        href: 'https://www.google.com/maps/d/viewer?mid=1K5ZxbnHUKRzTmY20fe9tocuPHBiSztQ&ll=-25.807393899217164%2C-49.8553047&z=8',
      },
    ],
    ...
    quandoIcones: [MapPin, Eye, Car, Link2],
    bignumbers: [
  ...
  },
  {
    ...
    // O vídeo é a mídia do hero e do card do carrossel (13/09/2026). As três
    // peças recortadas do cliente saíram do hero e da faixa abaixo de "Quando"
    // no mesmo dia e voltaram, uma por produto, nos cards de Formatos: lá elas
    // explicam o produto, que era o que faltava ao diagrama sem medida.
    //
    // O `?v=` existe porque o arquivo foi trocado por outro vídeo (a fachada da
    // Vanguard, 14/09/2026) no mesmo caminho: `/media/` sai com cache de sete
    // dias, e sem uma URL nova quem já tinha aberto a página seguia vendo o
    // vídeo anterior. Subir a versão ao trocar o arquivo de novo.
    video: '/media/plataformas/digital-signage.mp4?v=2',
    quando: [
    ...
    quandoIcones: [Building2, Megaphone, Network],
    ...
    formats: [
      ...
      // Enquanto isso, quem ilustra o card é a foto real (`image`), no lugar
      // do diagrama; os PNG originais ficam em
      // public/media/images/digital-signage, fora do versionamento.
      {
        aspect: '16/9',
        dims: 'Fachada',
        top: 'LED',
        ...
        image: '/media/digital-signage/fachada-digital-v2.webp',
        imageAlt:
          'Painel de LED vertical da Outdoormídia em frente à loja da Vanguard, ao lado do letreiro da fachada',
      },
      {
        aspect: '16/9',
        dims: 'Posto',
        ...
        image: '/media/digital-signage/posto-de-combustivel-v2.webp',
        imageAlt: 'Painel de LED vertical da Outdoormídia na fachada da Audiotec, exibindo a marca da loja',
      },
      {
        aspect: '3/1',
        dims: 'Passagem',
        ...
        image: '/media/digital-signage/passagem-digital-v2.webp',
        imageAlt: 'Painel de LED da Outdoormídia junto ao acesso de uma clínica, exibindo a marca e o contato do consultório',
      },
    ],
...
// Slugs que já atenderam por uma plataforma e seguem gravados nos cases do
// Firestore. A rota redireciona (next.config.mjs); aqui é o que mantém o case
// antigo válido no admin e visível na página nova.
export const SLUGS_LEGADOS = {
  'midia-indoor': ['shoppings'],
}

export function slugsDaPlataforma(slug) {
  return [slug, ...(SLUGS_LEGADOS[slug] ?? [])]
}

// O slug de hoje para um gravado antes da troca: o editor do admin marca a
// plataforma certa e regrava o case já com o slug novo ao salvar.
export function slugAtual(slug) {
  return Object.keys(SLUGS_LEGADOS).find((atual) => SLUGS_LEGADOS[atual].includes(slug)) ?? slug
}
