// Fonte única dos dados institucionais (NAP: nome, endereço, telefone).
//
// Consumido pelo JSON-LD de LocalBusiness (components/widgets/JsonLd.jsx) e
// pelo llms.txt. Alterar aqui reflete nos dois — nunca repetir esses valores
// soltos em componente ou página.
//
// Campo vazio é omitido do JSON-LD de propósito: dado errado em structured data
// vira endereço/horário errado no Google e no perfil da empresa. Melhor ausente
// que inventado.

export const EMPRESA = {
  nome: 'Outdoormídia',
  slogan: 'Toda Hora em Todo Lugar',
  descricao:
    'Empresa de mídia Out of Home (OOH) no Sul do Brasil desde 1959. Outdoors digitais, front lights, mídia indoor, aeroporto, rodovias, mobiliário urbano e mídia móvel no Paraná e em Santa Catarina.',
  servico: 'mídia exterior Out of Home (OOH)',
  fundacao: '1959',

  // TODO(cliente): falta o e-mail do encarregado de dados (LGPD, art. 41). Sem
  // `encarregado`, o canal do titular na Política de Privacidade vira o `email`
  // abaixo. Campo vazio é omitido — dado jurídico ausente é melhor que inventado.
  razaoSocial: 'Outdoormídia Locação de Espaços para Publicidade LTDA',
  cnpj: '04.686.938/0001-68',
  encarregado: '',

  telefone: '+554132076400',
  telefoneExibicao: '+55 41 3207-6400',

  // TODO(cliente): falta o telefone comercial de Santa Catarina. Enquanto
  // vazio, o rodapé e o JSON-LD mostram só o de Curitiba — número errado em
  // rodapé e em structured data vira ligação perdida.
  telefoneSc: '',
  telefoneScExibicao: '',
  whatsapp: '+5541998350210',
  whatsappExibicao: '+55 41 99835-0210',
  email: 'contato@outdoormidia.com.br',

  // Canal do RH, confirmado em 25/08/2026. Currículo que cai no `email` acima,
  // que é comercial, se perde: candidatura vai para cá.
  emailRh: 'rh@outdoormidia.com.br',

  endereco: {
    logradouro: 'Avenida Nossa Senhora Aparecida, 1260',
    cep: '80310-100',
    cidade: 'Curitiba',
    estado: 'PR',
    pais: 'BR',
  },

  // TODO(cliente): confirmar o horário de atendimento comercial. Formato:
  // { dias: ['Monday', ...], abre: '08:00', fecha: '18:00' }
  // Enquanto vazio, o openingHoursSpecification é omitido do JSON-LD.
  horarios: [],

  // TODO(cliente): coordenadas da sede (latitude e longitude), para o `geo`
  // do LocalBusiness. É o que amarra o negócio ao ponto no mapa nos motores
  // de busca local. Enquanto vazio, o campo é omitido do JSON-LD.
  geo: { latitude: '', longitude: '' },

  // As praças nomeadas em todo o site, cidade a cidade. Vai para o
  // `areaServed` do JSON-LD e para a linha "Praças atendidas" do llms.txt.
  areaServida: [
    'Curitiba',
    'Região Metropolitana de Curitiba',
    'Litoral do Paraná',
    'Joinville',
    'Itajaí',
    'Balneário Camboriú',
    'Paraná',
    'Santa Catarina',
  ],

  // Fonte única dos perfis sociais: o rodapé e o `sameAs` do JSON-LD leem
  // daqui. A ordem é a do briefing da reta final: Instagram, LinkedIn,
  // YouTube, Facebook. O X saiu do site no mesmo briefing.
  //
  // TODO(cliente): confirmar os perfis de YouTube e Facebook — os dois foram
  // montados a partir do handle do Instagram e ainda não foram verificados.
  redes: {
    instagram: 'https://www.instagram.com/outdoormidia/',
    linkedin: 'https://br.linkedin.com/company/outdoormidia',
    youtube: 'https://www.youtube.com/@outdoormidia',
    facebook: 'https://www.facebook.com/outdoormidia',
  },
}

// Versao de cada idioma. So descricao e servico: o resto e dado cadastral e
// nome proprio, que nao se traduz em structured data.
import { porLocale } from '@/lib/i18n/overlay'
import { EMPRESA as en } from '@/lib/i18n/content/empresa.en'
import { EMPRESA as es } from '@/lib/i18n/content/empresa.es'
import { EMPRESA as zh } from '@/lib/i18n/content/empresa.zh'

const conteudo = porLocale(EMPRESA, { en, es, zh })

export function getEmpresa(locale) {
  return conteudo(locale)
}
