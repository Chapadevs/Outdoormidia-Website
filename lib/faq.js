...
export const FAQS = [
  ...
  },
  {
    ...
    a: [
      'Sim. O time comercial envia a versão completa mediante solicitação, com praças, formatos e especificações de cada plataforma.',
    ],
  },
  {
    categoria: 'Como começar',
    id: 'como-receber-proposta',
  ...
  },
  {
    ...
    a: [
      'Depende da região e do objetivo. Um exemplo de referência: um circuito de 40 faces entrega cerca de 8,4 milhões de impactos em 7 dias, 18 milhões em 15 dias e 36 milhões em 30 dias.',
    ],
  },
  ...
  // o que está incluso.
  {
    ...
    a: [
      'O investimento varia conforme formato, região, quantidade de pontos e período. A proposta é montada a partir do seu objetivo e do orçamento disponível, e você recebe as opções que cabem no que pretende investir.',
    ],
  },
  {
    categoria: 'Contratação',
...
const PERGUNTAS_HOME = [
  ...
  'quanto-custa',
  'duracao-minima',
  'concorrente-mesmo-painel',
  'comprovacao',
  'como-receber-proposta',
]

export const FAQS_HOME = PERGUNTAS_HOME.map((id) => FAQS.find((f) => f.id === id))

// A selecao da home casava pelo texto completo da pergunta, o que quebraria
// na primeira traducao: o find devolveria undefined nas oito. Passou a casar
// por id, que e o mesmo nos quatro idiomas.
export function faqsHomeDe(faqs) {
  return PERGUNTAS_HOME.map((id) => faqs.find((f) => f.id === id))
}

// Versao traduzida de cada idioma. O overlay traz so os campos de texto; id,
// ordem e estrutura continuam vindo daqui.
import { porLocale } from '@/lib/i18n/overlay'
import * as en from '@/lib/i18n/content/faq.en'
import * as es from '@/lib/i18n/content/faq.es'
import * as zh from '@/lib/i18n/content/faq.zh'

const faqsDe = porLocale(FAQS, { en: en.FAQS, es: es.FAQS, zh: zh.FAQS })
const categoriasDe = porLocale(CATEGORIAS_FAQ, {
  en: en.CATEGORIAS_FAQ,
  es: es.CATEGORIAS_FAQ,
  zh: zh.CATEGORIAS_FAQ,
})

export function getFaqs(locale) {
  return faqsDe(locale)
}

export function getCategoriasFaq(locale) {
  return categoriasDe(locale)
}

export function getFaqsHome(locale) {
  return faqsHomeDe(faqsDe(locale))
}
