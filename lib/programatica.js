...
// Versao de cada idioma. Siglas de mercado (DSP, SSP, PD, PG, NG, JBP) e os
// nomes ja em ingles nao entram no overlay: caem no portugues, que os traz
// grafados como o mercado os usa.
import { porLocale } from '@/lib/i18n/overlay'
import * as pgEn from '@/lib/i18n/content/programatica.en'
import * as pgEs from '@/lib/i18n/content/programatica.es'
import * as pgZh from '@/lib/i18n/content/programatica.zh'

const conteudoDado = porLocale(DADO_MERCADO, {
  en: pgEn.DADO_MERCADO,
  es: pgEs.DADO_MERCADO,
  zh: pgZh.DADO_MERCADO,
})
const conteudoModelos = porLocale(MODELOS, { en: pgEn.MODELOS, es: pgEs.MODELOS, zh: pgZh.MODELOS })
const conteudoFluxo = porLocale(FLUXO, { en: pgEn.FLUXO, es: pgEs.FLUXO, zh: pgZh.FLUXO })
const conteudoGlossario = porLocale(GLOSSARIO, {
  en: pgEn.GLOSSARIO,
  es: pgEs.GLOSSARIO,
  zh: pgZh.GLOSSARIO,
})

export function getDadoMercado(locale) {
  return conteudoDado(locale)
}

export function getModelos(locale) {
  return conteudoModelos(locale)
}

export function getFluxo(locale) {
  return conteudoFluxo(locale)
}

export function getGlossario(locale) {
  return conteudoGlossario(locale)
}
