...
export const PERGUNTAS = [
  ...
  },
  {
    ...
    fragil: {
      ...
      link: { label: 'Regiões e cobertura', href: '/solucoes/regioes-cobertura' },
    },
  },
  {
...
// Os cinco rotulos da nota do slider. Viviam soltos dentro de rotuloDaNota,
// onde nenhum overlay os alcancava.
...
// Rotulo da faixa de perguntas de cada grupo ("Perguntas 01-02").
export const META_PERGUNTAS = 'Perguntas'

export function degrauDoScore(score, locale) {
  const degraus = getDegraus(locale)
  return degraus.find((degrau) => score <= degrau.max) ?? degraus[degraus.length - 1]
}

export function rotuloDaNota(nota, locale) {
  if (nota === null || nota === undefined) return ''
  const rotulos = getRotulosNota(locale)
  if (nota <= 2) return rotulos[0]
  if (nota <= 4) return rotulos[1]
  if (nota <= 6) return rotulos[2]
  if (nota <= 8) return rotulos[3]
  return rotulos[4]
}
...
export function gruposDePerguntas(locale) {
  const grupos = getGrupos(locale)
  const perguntas = getPerguntas(locale)
  const meta = getMetaPerguntas(locale)
  ...
  return grupos.map((grupo, gi) => {
    ...
    return { ...grupo, itens, meta: `${meta} ${primeira}-${ultima}` }
  })
}

// Versao de cada idioma. As classes de estilo dos degraus, os href e os
// limites numericos das faixas continuam vindo daqui.
import { porLocale } from '@/lib/i18n/overlay'
import * as dgEn from '@/lib/i18n/content/diagnostico.en'
import * as dgEs from '@/lib/i18n/content/diagnostico.es'
import * as dgZh from '@/lib/i18n/content/diagnostico.zh'

const conteudoGrupos = porLocale(GRUPOS, { en: dgEn.GRUPOS, es: dgEs.GRUPOS, zh: dgZh.GRUPOS })
const conteudoPerguntas = porLocale(PERGUNTAS, {
  en: dgEn.PERGUNTAS,
  es: dgEs.PERGUNTAS,
  zh: dgZh.PERGUNTAS,
})
const conteudoDegraus = porLocale(DEGRAUS, { en: dgEn.DEGRAUS, es: dgEs.DEGRAUS, zh: dgZh.DEGRAUS })
const conteudoEscala = porLocale(ESCALA, { en: dgEn.ESCALA, es: dgEs.ESCALA, zh: dgZh.ESCALA })
const conteudoRotulos = porLocale(ROTULOS_NOTA, {
  en: dgEn.ROTULOS_NOTA,
  es: dgEs.ROTULOS_NOTA,
  zh: dgZh.ROTULOS_NOTA,
})
const conteudoMeta = porLocale(META_PERGUNTAS, {
  en: dgEn.META_PERGUNTAS,
  es: dgEs.META_PERGUNTAS,
  zh: dgZh.META_PERGUNTAS,
})

export function getGrupos(locale) {
  return conteudoGrupos(locale)
}

export function getPerguntas(locale) {
  return conteudoPerguntas(locale)
}

export function getDegraus(locale) {
  return conteudoDegraus(locale)
}

export function getEscala(locale) {
  return conteudoEscala(locale)
}

export function getRotulosNota(locale) {
  return conteudoRotulos(locale)
}

export function getMetaPerguntas(locale) {
  return conteudoMeta(locale)
}
