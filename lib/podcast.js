...
// Versao de cada idioma. O nome do programa (`title` de PODCAST) nao entra no
// overlay: nome proprio de projeto nao se traduz.
import { porLocale } from '@/lib/i18n/overlay'
import { PODCAST as podcastEn, EPISODIOS as episodiosEn } from '@/lib/i18n/content/podcast.en'
import { PODCAST as podcastEs, EPISODIOS as episodiosEs } from '@/lib/i18n/content/podcast.es'
import { PODCAST as podcastZh, EPISODIOS as episodiosZh } from '@/lib/i18n/content/podcast.zh'

const conteudoPodcast = porLocale(PODCAST, { en: podcastEn, es: podcastEs, zh: podcastZh })
const conteudoEpisodios = porLocale(EPISODIOS, { en: episodiosEn, es: episodiosEs, zh: episodiosZh })

export function getPodcast(locale) {
  return conteudoPodcast(locale)
}

export function getEpisodios(locale) {
  return conteudoEpisodios(locale)
}
