import { DEFAULT_LOCALE, IDIOMAS, LOCALES, TAG_OG } from '@/i18n/routing'
...
export const PAGINAS = [
  ...
  },
  {
    ...
    titulo: 'Ambiental',
    resumo:
      'Praça de Carregamento Elétrico, Praça de Conveniência Batel e Jardim Vertical, o ciclo da lona e as práticas que reduzem o impacto da operação.',
    priority: 0.4,
  ...
  },
  {
    ...
    titulo: 'Social',
    resumo:
      'Corajosamente Éticos, Loja OM do Bem e a Mídia Regenerativa da Praça de Conveniência Batel, primeiro ativo de mídia exterior ligado à Muralha Digital de Curitiba.',
    priority: 0.5,
    changeFrequency: 'yearly',
  },
  {
  ...
  },
  {
    ...
    resumo:
      'Diagnóstico de presença, Sua marca no OOH, Melhores práticas e FAQ: as ferramentas para resolver sozinho antes de falar com o comercial.',
    priority: 0.8,
    changeFrequency: 'monthly',
    destaque: true,
  },
  {
    ...
    resumo:
      'Suba a sua logo em PNG e veja a sua marca aplicada em um painel real da Outdoormídia, no Paraná e em Santa Catarina. Baixe a imagem em segundos.',
    priority: 0.7,
    changeFrequency: 'monthly',
  },
  {
...
// Canonical e hreflang de uma rota, para o `alternates` de cada página.
//
// Sem `languages`, as versões EN/ES/ZH existem mas o Google não sabe que são a
// mesma página em outro idioma: cada uma disputa sozinha, e a versão em
// português (que é a indexada) tende a ser a única servida. É o par canonical
// + hreflang que amarra as quatro.
//
// `x-default` aponta para a versão sem prefixo, que é a portuguesa: é ela que
// responde a quem chega sem idioma declarado.
export function caminhoLocalizado(path, locale) {
  if (locale === DEFAULT_LOCALE) return path
  return path === '/' ? `/${locale}` : `/${locale}${path}`
}

export function alternatesDe(path, locale) {
  return {
    canonical: caminhoLocalizado(path, locale),
    languages: {
      ...Object.fromEntries(IDIOMAS.map((i) => [i.tag, caminhoLocalizado(path, i.code)])),
      'x-default': path,
    },
  }
}

// Metadata inteira de uma rota: title, description, canonical + hreflang e o
// Open Graph com url, siteName e locale. Cada página montava o próprio bloco,
// e o resultado divergia do layout: o `openGraph` da página substitui o do
// layout por inteiro (o Next não faz merge profundo), então as rotas internas
// saíam sem og:url e sem og:site_name. Aqui é um lugar só, e a imagem de OG
// entra em todas as páginas no dia em que o asset chegar.
//
// `somentePt` é para conteúdo que só existe em português (posts do blog):
// o canonical de qualquer idioma aponta para a URL sem prefixo e não sai
// hreflang, senão /en/blog/x declara ser a versão inglesa de um texto em
// português.
//
// `openGraph` entra por cima do bloco padrão: é como o post declara
// `type: 'article'`, datas e autor sem repetir o resto.
export function metaDe({
  ...
  const canonical = somentePt ? path : caminhoLocalizado(path, locale)
  const ogLocale = somentePt ? DEFAULT_LOCALE : locale

  return {
    title: titulo,
    description: descricao,
    alternates: somentePt ? { canonical } : alternatesDe(path, locale),
    ...
    openGraph: {
      ...
      locale: TAG_OG[ogLocale],
      alternateLocale: somentePt
        ? []
        : LOCALES.filter((l) => l !== locale).map((l) => TAG_OG[l]),
      type: 'website',
