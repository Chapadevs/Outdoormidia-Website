...
//
// `tecnologias` marca o produto de dupla tecnologia. Até 12/09/2026 o card
// trazia um seletor ESTÁTICO/DIGITAL para trocar a ficha técnica; ele saiu a
// pedido do cliente e deu lugar a `imagemPor`, o mapa tecnologia → foto real
// que `ProdutoCard` sobe lado a lado (uma coluna só quando existe foto de uma
// tecnologia só). `specsPor` segue guardando a ficha de cada tecnologia; onde
// a ficha ainda não veio, a linha de specs some em vez de mostrar medida
// inventada.
//
// Plataforma de uma tecnologia só (`tecnologia` em `lib/platforms.js`, hoje
// Outdoor Digital) recebe o produto restrito a ela: só a foto, os pontos e o
// texto dessa tecnologia (`textPor`), para página de painel digital não subir
// painel estático. Decisão de 14/09/2026, a pedido do cliente, que tirou de
// Outdoor Digital o Top Sight (só tem foto do estático), o Billboard e o lado
// estático do Topo de Prédio. Em Front Light o produto segue inteiro.
//
...
export const PRODUTOS = [
  {
    ...
    imagemPor: {
      estatico: {
        src: '/media/produtos/top-sight-estatico.webp',
        alt: 'Painel Top Sight estático da Outdoormídia à beira de via, em Curitiba',
      },
    },
  ...
  },
  {
    ...
    imagemPor: {
      digital: {
        src: '/media/produtos/poster-sight-digital.webp',
        alt: 'Painel Poster Sight digital da Outdoormídia com campanha em LED sobre a via',
      },
    },
  ...
  },
  {
    ...
    imagemPor: {
      digital: {
        src: '/media/produtos/billboard-digital.webp',
        alt: 'Estrutura Billboard da Outdoormídia iluminada à noite, com campanha em grande formato',
      },
    },
    plataformas: ['front-lights'],
  },
  {
    ...
    textPor: {
      digital:
        'O formato mais alto do portfólio, no alto das duas cidades mais relevantes de Santa Catarina. Em Balneário Camboriú, painel digital em ponto nobre da Av. Brasil. Em Joinville, painel digital com conteúdo em tempo real. Visibilidade premium para marcas que buscam protagonismo na paisagem, onde só a Outdoormídia chega.',
    },
    pontos: [
      { name: 'Balneário Camboriú', endereco: 'Av. Brasil, 3830', tecnologia: 'digital' },
      { name: 'Joinville · digital', endereco: 'Rua João Colin, 1875', tecnologia: 'digital' },
      // TODO(cliente): endereço do estático de Joinville (pendência 9 do
      // handoff). Sem `endereco` o ponto não é listado.
      { name: 'Joinville · estático', endereco: null, tecnologia: 'estatico' },
    ],
    imagemPor: {
      estatico: {
        src: '/media/produtos/topo-de-predio-estatico.webp',
        alt: 'Painel Topo de Prédio estático da Outdoormídia no alto de um edifício',
      },
      digital: {
        src: '/media/produtos/topo-de-predio-digital.webp',
        alt: 'Painel Topo de Prédio digital da Outdoormídia no alto de um edifício',
      },
    },
    plataformas: ['outdoors-digitais', 'front-lights'],
  },
  ...
  },
  {
    ...
    // A foto do acervo é retrato (900×1600): na caixa 16/9 do card sobrava só o
    // meio da tela. O corte 4:3 enquadra a tela inteira e o topo da estrutura.
    image: '/media/shoppings/produto-totem-4x3.webp',
    imageAlt: 'Totem digital da Outdoormídia no Park Shopping Boulevard',
    imageRatio: '4/3',
  ...
  },
  {
    ...
    image: '/media/tipos-de-midia/banca-horizontal.jpg',
    imageAlt: 'Banca Horizontal da Outdoormídia com painel digital em esquina de Curitiba',
    plataformas: ['mub'],
  },
  {
    slug: 'banca-vertical',
    ...
    image: '/media/tipos-de-midia/banca-vertical.jpg',
    imageAlt: 'Banca Vertical da Outdoormídia com painel digital em calçada movimentada',
    plataformas: ['mub'],
  },
  {
    ...
    specs: '768×1024 px · MP4 · 10 segundos',
    image: '/media/tipos-de-midia/relogio-digital.jpg',
    imageAlt: 'Relógio Digital da Outdoormídia exibindo hora, temperatura e anúncio em posto de combustível',
    plataformas: ['mub'],
  },
  {
    slug: 'bike-midia',
    ...
    // As duas fotos de Mídia Móvel sobem em 4/3: a do Bus Mídia é um retrato
    // recortado de cima, com o anúncio e o número do ônibus, e em 16/9 sobraria
    // só a lona sem o ônibus em volta. A da Bike acompanha para os dois cards
    // ficarem da mesma altura; ela é 16/9 inteira e o corte central mantém o
    // trio, que está no centro do quadro.
    image: '/media/midia-movel/bike-midia.webp',
    imageAlt: 'Trio de Bike Mídia da Outdoormídia circulando no gramado de um parque de Curitiba, com a roda-gigante ao fundo',
    imageRatio: '4/3',
  ...
  },
  {
    ...
    image: '/media/midia-movel/bus-midia.webp',
    imageAlt: 'Traseira de ônibus com campanha do Bus Mídia da Outdoormídia ocupando o vidro inteiro',
    imageRatio: '4/3',
    plataformas: ['midia-movel'],
  },
  {
...
// Versao de cada idioma. Nome de produto nao entra no overlay: nome oficial
// nao se traduz. `plataformas`, `tecnologias`, `image` e endereco tambem
// continuam vindo daqui.
import { porLocale } from '@/lib/i18n/overlay'
import { PRODUTOS as en } from '@/lib/i18n/content/produtos.en'
import { PRODUTOS as es } from '@/lib/i18n/content/produtos.es'
import { PRODUTOS as zh } from '@/lib/i18n/content/produtos.zh'

const conteudo = porLocale(PRODUTOS, { en, es, zh })

export function getProdutos(locale) {
  return conteudo(locale)
}

export function getProdutosPorPlataformaLocale(slug, locale, tecnologia) {
  const lista = conteudo(locale).filter((p) => p.plataformas.includes(slug))
  return tecnologia ? lista.map((p) => restringirTecnologia(p, tecnologia)) : lista
}

function restringirTecnologia(produto, tecnologia) {
  const { textPor, imagemPor, pontos } = produto
  return {
    ...produto,
    text: textPor?.[tecnologia] ?? produto.text,
    imagemPor: imagemPor?.[tecnologia] ? { [tecnologia]: imagemPor[tecnologia] } : imagemPor,
    pontos: pontos?.filter((ponto) => !ponto.tecnologia || ponto.tecnologia === tecnologia),
  }
}
