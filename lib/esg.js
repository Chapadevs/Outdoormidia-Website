...
// entrega. Os três integram a carteira Mídia Regenerativa.
//
// TODO(Imagine): falta a foto da Praça de Carregamento Elétrico (16/9,
// ≥1600px) — é a única que ainda bloqueia a publicação da página. Praça de
// Conveniência Batel e Jardim Vertical já têm foto aplicada.
export const AMBIENTAL_REALIDADE = [
  {
    slug: 'praca-de-carregamento-eletrico',
    tag: 'Regenerativo · Mobilidade',
    ...
    text: [
      ...
      'Reconhecido com o Ouro na categoria Projetos Sustentáveis do Prêmio Central de Outdoor 2025.',
    ],
  },
  {
    slug: 'praca-conveniencia-batel',
    tag: 'Regenerativo · Cidade',
    title: 'Praça de Conveniência Batel',
...
// Versao de cada idioma. O overlay traz so `tag`, `title` e `text`: slug,
// imagem e componente de icone continuam vindo daqui.
import { porLocale } from '@/lib/i18n/overlay'
import * as esgEn from '@/lib/i18n/content/esg.en'
import * as esgEs from '@/lib/i18n/content/esg.es'
import * as esgZh from '@/lib/i18n/content/esg.zh'

const conteudoRealidade = porLocale(AMBIENTAL_REALIDADE, {
  en: esgEn.AMBIENTAL_REALIDADE,
  es: esgEs.AMBIENTAL_REALIDADE,
  zh: esgZh.AMBIENTAL_REALIDADE,
})
const conteudoPraticas = porLocale(AMBIENTAL_PRATICAS, {
  en: esgEn.AMBIENTAL_PRATICAS,
  es: esgEs.AMBIENTAL_PRATICAS,
  zh: esgZh.AMBIENTAL_PRATICAS,
})
const conteudoPilares = porLocale(GOV_PILARES, {
  en: esgEn.GOV_PILARES,
  es: esgEs.GOV_PILARES,
  zh: esgZh.GOV_PILARES,
})

export function getAmbientalRealidade(locale) {
  return conteudoRealidade(locale)
}

export function getAmbientalPraticas(locale) {
  return conteudoPraticas(locale)
}

export function getGovPilares(locale) {
  return conteudoPilares(locale)
}
