...
export const ATUALIZADO_EM = '2026-09-14'
...
export const PRIVACIDADE = [
  ...
  },
  {
    ...
    lista: [
      ...
      'Cookies e armazenamento local necessários: usados só para o site funcionar, como lembrar a sua escolha no aviso de cookies.',
      'Cookies de medição, só com o seu aceite: estatísticas de uso das páginas pelo Google Analytics, como páginas visitadas, origem da visita e cliques nos botões de contato. O endereço IP é truncado pelo Google antes de ser guardado.',
    ],
  },
  {
    id: 'audiencia',
  ...
  },
  {
    ...
    paragrafos: [
      'Os cookies e o armazenamento local necessários servem só para o site funcionar: por exemplo, para lembrar a sua escolha no aviso de cookies e para guardar temporariamente, dentro do seu próprio navegador, o briefing recém-enviado, de modo a montar a mensagem de WhatsApp da tela de confirmação.',
      'Os cookies de medição são do Google Analytics e só entram na página se você clicar em Aceitar no aviso de cookies. Eles nos dizem quais páginas são mais visitadas, de onde as visitas chegam e quantas terminam em contato, de forma agregada. Se você escolher Só necessários, nenhum script de medição é carregado. Para mudar a escolha depois, apague os dados deste site nas configurações do navegador e o aviso volta a aparecer.',
...
// Versao de cada idioma. `id` e `ATUALIZADO_EM` continuam vindo daqui.
//
// As traducoes sao de cortesia: o texto que vale juridicamente e o portugues.
// Revisao do juridico do cliente precisa alcancar os quatro idiomas.
import { porLocale } from '@/lib/i18n/overlay'
import * as legalEn from '@/lib/i18n/content/legal.en'
import * as legalEs from '@/lib/i18n/content/legal.es'
import * as legalZh from '@/lib/i18n/content/legal.zh'

const conteudoPrivacidade = porLocale(PRIVACIDADE, {
  en: legalEn.PRIVACIDADE,
  es: legalEs.PRIVACIDADE,
  zh: legalZh.PRIVACIDADE,
})
const conteudoTermos = porLocale(TERMOS, {
  en: legalEn.TERMOS,
  es: legalEs.TERMOS,
  zh: legalZh.TERMOS,
})
const conteudoContatoPrivacidade = porLocale(CONTATO_PRIVACIDADE, {
  en: legalEn.CONTATO_PRIVACIDADE,
  es: legalEs.CONTATO_PRIVACIDADE,
  zh: legalZh.CONTATO_PRIVACIDADE,
})
const conteudoContatoTermos = porLocale(CONTATO_TERMOS, {
  en: legalEn.CONTATO_TERMOS,
  es: legalEs.CONTATO_TERMOS,
  zh: legalZh.CONTATO_TERMOS,
})

export function getPrivacidade(locale) {
  return conteudoPrivacidade(locale)
}

export function getTermos(locale) {
  return conteudoTermos(locale)
}

export function getContatoPrivacidade(locale) {
  return conteudoContatoPrivacidade(locale)
}

export function getContatoTermos(locale) {
  return conteudoContatoTermos(locale)
}
