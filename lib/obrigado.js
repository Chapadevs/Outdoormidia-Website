...
// formulário antes desta página existir.
//
// `talentos` ficou sem produtor em 25/08/2026: o banco de talentos passou a ser
// um Google Forms do cliente, e nada mais leva a esta origem. A entrada segue
// aqui porque a URL continua respondendo, e o caminho que ela oferece, o e-mail
// do RH, é o real — nunca dizer que guardamos o perfil.
...
// Versao de cada idioma. O overlay traz so os campos de texto; `num` e `href`
// continuam vindo daqui.
import { porLocale } from '@/lib/i18n/overlay'
import { ORIGENS as origensEn, SUGESTOES as sugestoesEn } from '@/lib/i18n/content/obrigado.en'
import { ORIGENS as origensEs, SUGESTOES as sugestoesEs } from '@/lib/i18n/content/obrigado.es'
import { ORIGENS as origensZh, SUGESTOES as sugestoesZh } from '@/lib/i18n/content/obrigado.zh'

const conteudoOrigens = porLocale(ORIGENS, { en: origensEn, es: origensEs, zh: origensZh })
const conteudoSugestoes = porLocale(SUGESTOES, { en: sugestoesEn, es: sugestoesEs, zh: sugestoesZh })

export function getOrigens(locale) {
  return conteudoOrigens(locale)
}

export function getSugestoes(locale) {
  return conteudoSugestoes(locale)
}
